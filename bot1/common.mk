...
INCLUDE=$(foreach dir,$(INCDIR) $(EXTRA_INCDIR) $(SHARED_INCDIR),-iquote"$(dir)")
...
SHARED_CSRC=$(foreach cext,$(CEXTS),$(call rwildcard, $(SHARED_SRCDIR),*.$(cext), $1))
SHARED_COBJ=$(addprefix $(SHARED_BINDIR)/,$(patsubst $(SHARED_SRCDIR)/%,%.o,$(call SHARED_CSRC, $1)))
...
$(SHARED_BINDIR)/%.$1.o: $(SHARED_SRCDIR)/%.$1
$(SHARED_BINDIR)/%.$1.o: $(SHARED_SRCDIR)/%.$1 $(DEPDIR)/$(basename $1).d
	$(VV)mkdir -p $$(dir $$@)
	$(MAKESHAREDDEPFOLDER)
	$$(call test_output_2,Compiled $$< ,$(CC) -c $(INCLUDE) -iquote"$(SHARED_INCDIR)/$$(dir $$*)" $(CFLAGS) $(EXTRA_CFLAGS) $(SHARED_DEPFLAGS) -o $$@ $$<,$(OK_STRING))
...
$(SHARED_BINDIR)/%.$1.o: $(SHARED_SRCDIR)/%.$1 $(DEPDIR)/$(basename $1).d
	$(VV)mkdir -p $$(dir $$@)
	$(MAKESHAREDDEPFOLDER)
	$$(call test_output_2,Compiled $$< ,$(CXX) -c $(INCLUDE) -iquote"$(SHARED_INCDIR)/$$(dir $$*)" $(CXXFLAGS) $(EXTRA_CXXFLAGS) $(SHARED_DEPFLAGS) -o $$@ $$<,$(OK_STRING))
	$(RENAMESHAREDDEPENDENCYFILE)
endef
